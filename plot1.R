...
##Would like to do it from the original zip file without unzipping
##zipFileCon<-unz(zipFile,dataFile)
##open(zipFileCon)
##powerConsDF <- read.table(pipe('grep "^[1-2]/2/2007\\|^Date" zipFileCon)'), sep=";", na.strings="?", header=TRUE)
##close(zipFileCon)
